...
df_initial=pd.read_json('df_gen1.json',orient='split')
...
for idx, row in data.iterrows():
    ...
    if row.operation!=None:
        ...
        #os.chdir('/dss/dsshome1/lxc09/ge28quz2/1_proj/20230215_df_initial/calc_1G/smi_logs')
        current_smi=row.smi
        print(row.smi)
        if current_smi!='c1ccccc1':
            side_group=df_initial.loc[df_initial.smi==current_smi,'operation'].values[0].split('>>')[1][6:-1]
            if side_group in side_groups:
                groups.append(side_group)
...
#os.chdir('..')
os.chdir(cwd)
df_sidegroups.to_json('df_sidegroups_initial.json',orient='split')
